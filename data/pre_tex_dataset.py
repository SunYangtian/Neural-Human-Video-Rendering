...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        self.dir_A = opt.pose_texture_path
        self.A_paths = sorted(make_dataset(self.dir_A))   
        ...
        texture = Image.open(self.tex_paths[0]).convert('RGB') 
        ...
        assert(len(self.tex_paths) == self.dataset_size), \
            "nums of texture (%d) is not equal to image (%d)" % (len(self.tex_paths), self.dataset_size)
    ...
    def __getitem__(self, index):
        ...
        if A.width != self.tex_size * 4:
            A = A.resize((self.tex_size*4,self.tex_size*6))
        A_tensor = self.transform(A)
        A_tensor = tex_im2tensor(A_tensor, self.tex_size)
